...
%
%  Note: for targets on surface, only the principal value part is returned
%
%  Input arguments:
%    * S: surfer object
...
%       targinfo.patch_id (nt,) patch id of target, = -1 if off-surface
%       targinfo.uvs_targ (2,nt) local uv coordinates on patch if on-surface
%    * opts: options struct
%        opts.format - storage format: 'rsc' (default), 'csc', 'sparse'

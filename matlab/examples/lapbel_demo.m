% Solve the Laplace-Beltrami equation on an ellipsoid, against an analytic
...
plot(S, real(u));
colorbar
title('Laplace-Beltrami solution')
...
f = f - sum(f.*S.wts)/sum(S.wts);

ug = Kmat*gmres(Amat, f, [], 1e-12, 200);
ug = ug - sum(ug.*S.wts)/sum(S.wts);

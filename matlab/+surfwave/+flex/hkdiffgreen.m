...
[cf1,cf2] = surfwave.flex.besselkikdiff_etc_pscoefs(nterms);
cf1(1) = cf1(1)*r2logrfac;
kpow = k*k*(k.^(4*(0:(nterms-1)))); % k^2, k^6, k^10, ...
cf1 = cf1(:).*kpow(:); cf2 = cf2(:).*kpow(:);


jdiff = surfwave.flex.pseval(cf1,r4).*r2;
f = surfwave.flex.pseval(cf2,r4).*r2;

% differentiate power series to get derivatives
fac = 2+4*(0:(nterms-1)).';
jdiffd1 = surfwave.flex.pseval(cf1.*fac,r4).*rsus;
fd1 = surfwave.flex.pseval(cf2.*fac,r4).*rsus;
d = fac.*(fac-1)-fac;
jdiffd21 = surfwave.flex.pseval(cf1.*d,r4);
fd21 = surfwave.flex.pseval(cf2.*d,r4);
...
jdiffd321 = surfwave.flex.pseval(cf1.*d,r4).*rsus.*r2;
fd321 = surfwave.flex.pseval(cf2.*d,r4).*rsus.*r2;
% g4321 = g'''' - 6*g'''/r + 15*g''/r^2 - 15*g'/r^3 
d = fac.*(fac-1).*(fac-2).*(fac-3)-6*fac.*(fac-1).*(fac-2)+15*(fac.*(fac-1)-fac);
jdiffd4321 = surfwave.flex.pseval(cf1.*d,r4).*r2;
fd4321 = surfwave.flex.pseval(cf2.*d,r4).*r2;
% g54321 = g''''' - 10g''''/r + 45g'''/r^2 -105g''/r^3 + 105g'/r^4
d = fac.*(fac-1).*(fac-2).*(fac-3).*(fac-4) - ...
    10*fac.*(fac-1).*(fac-2).*(fac-3) + 45*fac.*(fac-1).*(fac-2) - ...
    105*(fac.*(fac-1)-fac);
jdiffd54321 = surfwave.flex.pseval(cf1.*d,r4).*rsus;
fd54321 = surfwave.flex.pseval(cf2.*d,r4).*rsus;

...
[iquad] = fmm3dbierouts(mex_id_, npatches, ixyzs, npts, nnz, row_ptr, col_ind, iquad, 1, npp1, 1, 1, nptsp1, nnz, nnzp1);
    ...
    mex_id_ = 'getnearquad_helm_rpcomb_neu_spmat(i int[x], i int[x], i int[x], i int[x], i int[x], i double[xx], i double[xx], i double[x], i dcomplex[x], i int[x], i int[x], i int[x], i int[x], i int[x], i double[x], i int[x], io dcomplex[xx], io int[x], io int[x])';
[wnear, irowind, icolind] = fmm3dbierouts(mex_id_, npatches, norders, ixyzs, iptype, npts, srccoefs, srcvals, eps, zpars, iquadtype, nnz, row_ptr, col_ind, iquad, rfac0, nquad, wnear, irowind, icolind, 1, npatches, npp1, npatches, 1, n9, npts, n12, npts, 1, 2, 1, 1, nptsp1, nnz, nnzp1, 1, 1, nquad, 4, nquad, nquad);
    spmat = cell(1,4);

...
%LAP3D.KERN standard Laplace layer potential kernels in 3D
% 
% Syntax: submat = lap3d.kern(zk,srcinfo,targingo,type,varargin)
%
% Let x be targets and y be sources for these formulas, with
% n_x and n_y the corresponding unit normals at those points.
%  
% Kernels based on G(x,y) = 1/(4*pi*|x-y|)
%
% D(x,y) = \nabla_{n_y} G(x,y)
% S(x,y) = G(x,y)
% S'(x,y) = \nabla_{n_x} G(x,y)
...
if strcmpi(type,'eval')
  ...
  % S
  [submats,grad] = lap3d.green(src,targ);
...
if strcmpi(type,'evalg')
  ...
  % S
  [submats,grad,hess] = lap3d.green(src,targ);

...
if isscalar(zk)
    if abs(zk) < 1e-6 % biharmonic (\Delta^2 u) 
        gfunc1 = @(s,t) chnk.flex2d.bhgreen(s,t);
        ...
        gfunc1 = @(s,t) chnk.flex2d.helmdiffgreen(zk1,s,t);
        gfunc2 = @(s,t) chnk.flex2d.helmdiffgreen(zk2,s,t);

        gfunc1f = @(s,t) chnk.flex2d.helmdiffgreen(zk1,s,t,true);
        gfunc2f = @(s,t) chnk.flex2d.helmdiffgreen(zk2,s,t,true);
    end
elseif length(zk) == 2
...
case {'free_plate_bcs'}
    ...
    case {'free_plate_bcs_var'} % variable coefficient BCs
    targnorm = targinfo.n;
    targtang = targinfo.d;
    targd2 = targinfo.d2;
    nu = varargin{1};
    a = targinfo.data(1,:);
    dadn = targinfo.data(2,:);
    dadt = targinfo.data(3,:);
    ...
    ((hess(:, :, 1).*nxtarg.*nxtarg + hess(:, :, 2).*(2*nxtarg.*nytarg) + hess(:, :, 3).*nytarg.*nytarg))) + ...
    (dadn(:)./a(:)).*firstbc + ...
    2*(1-nu)*(dadt(:)./a(:)).*(nxtarg.*tauxtarg.*hess(:,:,1) + (nxtarg.*tauytarg+nytarg.*tauxtarg).*hess(:,:,2) + nytarg.*tauytarg.*hess(:,:,3));
...
% kernels for the free plate integral equation with variable thickness
case {'free_plate_var'}
   srcnorm = srcinfo.n;
   srctang = srcinfo.d;
   targnorm = targinfo.n;
   ...
   targd2 = targinfo.d2;
   nu = varargin{1};
    a_targ = targinfo.data(1,:);
    dadn_targ = targinfo.data(2,:);
    dadt_targ = targinfo.data(3,:);   
    a = srcinfo.data(1,:);
    dadn = srcinfo.data(2,:);
    dadt = srcinfo.data(3,:);   
   ...
   K21 = kappatarg.*(1-nu).*((third(:, :, 1).*(nxtarg.*nxtarg.*nx) + third(:, :, 2).*(nxtarg.*nxtarg.*ny + 2*nxtarg.*nytarg.*nx) +...
        ...
        - (fourth(:, :, 1).*(nxtarg.*nxtarg.*nxtarg.*nx) + fourth(:, :, 2).*(nxtarg.*nxtarg.*nxtarg.*ny + 3*nxtarg.*nxtarg.*nytarg.*nx) + ...
          ...
          fourth(:, :, 5).*(tauytarg.*tauytarg.*nytarg.*ny)) ) - ...          
          (1+nu)/(4*pi).*((taux.*tauxtarg + tauy.*tauytarg)./(r2) - 2*(rx.*tauxtarg + ry.*tauytarg).*(rx.*taux + ry.*tauy)./(r2.^2)) + ... 
          (dadn_targ(:)./a_targ(:)).*(K11) + ...
          -2*(1-nu)*(dadt_targ(:)./a_targ(:)).*(nx.*(nxtarg.*tauxtarg.*third(:,:,1) + (nxtarg.*tauytarg+nytarg.*tauxtarg).*third(:,:,2) + nytarg.*tauytarg.*third(:,:,3)) + ...
         ny.*(nxtarg.*tauxtarg.*third(:,:,2) + (nxtarg.*tauytarg+nytarg.*tauxtarg).*third(:,:,3) + nytarg.*tauytarg.*third(:,:,4))) ...
         + 1/2*(1-nu)*(dadt_targ(:)./a_targ(:)).*hilb;
   ...
   K22 = (third(:, :, 1).*(nxtarg.*nxtarg.*nxtarg) + third(:, :, 2).*(3*nxtarg.*nxtarg.*nytarg) +...
       third(:, :, 3).*(3*nxtarg.*nytarg.*nytarg) + third(:, :, 4).*(nytarg.*nytarg.*nytarg))  +...
        ...
        + kappatarg.*(1-nu).*((hess(:, :, 1).*tauxtarg.*tauxtarg + hess(:, :, 2).*(2*tauxtarg.*tauytarg) + hess(:, :, 3).*tauytarg.*tauytarg)-...
           (hess(:, :, 1).*nxtarg.*nxtarg + hess(:, :, 2).*(2*nxtarg.*nytarg) + hess(:, :, 3).*nytarg.*nytarg)) + ...
        (dadn_targ(:)./a_targ(:)).*(K12) + ...
        2*(1-nu)*(dadt_targ(:)./a_targ(:)).*(nxtarg.*tauxtarg.*hess(:,:,1) + (nxtarg.*tauytarg+nytarg.*tauxtarg).*hess(:,:,2) + nytarg.*tauytarg.*hess(:,:,3)); 
   ...
   K21H = kappatarg.*(1-nu).*(-((1+ nu)/2).*((third(:, :, 1).*(tauxtarg.*tauxtarg.*taux) + third(:, :, 2).*(tauxtarg.*tauxtarg.*tauy + 2*tauxtarg.*tauytarg.*taux) +...
       third(:, :, 3).*(2*tauxtarg.*tauytarg.*tauy + tauytarg.*tauytarg.*taux) +...
        ...
        -(1+ nu)/2.*((fourth(:, :, 1).*(nxtarg.*nxtarg.*nxtarg.*taux) + ...
          ...
          fourth(:, :, 4).*(tauytarg.*tauytarg.*nxtarg.*tauy + 2*tauxtarg.*tauytarg.*nytarg.*tauy + tauytarg.*tauytarg.*nytarg.*taux) +...
         fourth(:, :, 5).*(tauytarg.*tauytarg.*nytarg.*tauy)) ) + ...
         (dadn_targ(:)./a_targ(:)).*(K11H) + ...
         -(1+ nu)/2*2*(1-nu)*(dadt_targ(:)./a_targ(:)).*(taux.*(nxtarg.*tauxtarg.*third(:,:,1) + (nxtarg.*tauytarg+nytarg.*tauxtarg).*third(:,:,2) + nytarg.*tauytarg.*third(:,:,3)) + ...
         tauy.*(nxtarg.*tauxtarg.*third(:,:,2) + (nxtarg.*tauytarg+nytarg.*tauxtarg).*third(:,:,3) + nytarg.*tauytarg.*third(:,:,4)));
...
case {'free_plate_eval'}
   ...
   K1 = -(grad(:, :, 1).*(nx) + grad(:, :, 2).*ny); 
   K1H = (1 + nu)/2.*(-(grad(:, :, 1).*(taux) + grad(:, :, 2).*tauy));                    % G_{tauy}
   K2 = val;
...
case {'free_plate_eval_var'}
   ...
   K1 = -(grad(:, :, 1).*(nx) + grad(:, :, 2).*ny); 
   K1H = (1 + nu)/2.*(-(grad(:, :, 1).*(taux) + grad(:, :, 2).*tauy)) + (1-nu)*val.*(dadt./a);                    % G_{tauy}
   K2 = val;
...
case {'free_plate_eval_lapgx'}
   ...
   [~, ~, ~, third1, fourth1] = gfunc1(src,targ);    
   [~, ~, ~, third2, fourth2] = gfunc2(src,targ);
   ...
   K1 = -((fourth(:,:,1) + fourth(:,:,3)).*nx + (fourth(:,:,2) + fourth(:,:,4)).*ny); 
   K1H = (1 + nu)/2.*(-((fourth(:,:,1) + fourth(:,:,3)).*taux + (fourth(:,:,2) + fourth(:,:,4)).*tauy));                    % G_{tauy}
   K2 = third(:,:,1) + third(:,:,3);

   K1H = K1H + (1-nu)*K2.*(dadt./a);
...
case {'free_plate_eval_lapgy'}
   ...
   K1 = -((fourth(:,:,2) + fourth(:,:,4)).*nx + (fourth(:,:,3) + fourth(:,:,5)).*ny); 
   K1H = (1 + nu)/2.*(-((fourth(:,:,2) + fourth(:,:,4)).*taux + (fourth(:,:,3) + fourth(:,:,5)).*tauy));                    % G_{tauy}
   K2 = third(:,:,2) + third(:,:,4);
...
case {'free_plate_eval_lapg'}
   ...
   K1 = -((third(:,:,1) + third(:,:,3)).*nx + (third(:,:,2) + third(:,:,4)).*ny); 
   K1H = (1 + nu)/2.*(-((third(:,:,1) + third(:,:,3)).*taux + (third(:,:,2) + third(:,:,4)).*tauy));                    % G_{tauy}
   K2 = hess(:,:,1) + hess(:,:,3);
...
case {'free_plate_eval_gyy'}
   ...
   K1 = -(third(:, :, 3).*(nx) + third(:, :, 4).*ny); 
   K1H = (1 + nu)/2.*(-(third(:, :, 3).*(taux) + third(:, :, 4).*tauy));                    % G_{tauy}
   K2 = hess(:,:,3);
...
case {'free_plate_eval_gxy'}
   ...
   K1 = -(third(:, :, 2).*(nx) + third(:, :, 3).*ny); 
   K1H = (1 + nu)/2.*(-(third(:, :, 2).*(taux) + third(:, :, 3).*tauy));                    % G_{tauy}
   K2 = hess(:,:,2);
...
case {'free_plate_eval_gxx'}
   ...
   K1 = -(third(:, :, 1).*(nx) + third(:, :, 2).*ny); 
   K1H = (1 + nu)/2.*(-(third(:, :, 1).*(taux) + third(:, :, 2).*tauy));                    % G_{tauy}
   K2 = hess(:,:,1);
...
case {'supported_plate_smooth'}
    ...
    [~, ~, ~, third, forth, fifth] = chnk.flex2d.bhgreen(src,targ);
    
    submat = -(fifth(:,:,1).*nxtarg.^2.*nx.^3 + ...
          fifth(:,:,2).*(2*nxtarg.*nytarg.*nx.^3+3*nxtarg.^2.*nx.^2.*ny) + ...
          fifth(:,:,3).*(nytarg.^2.*nx.^3 + 6*nxtarg.*nytarg.*nx.^2.*ny+3.*nxtarg.^2.*nx.*ny.^2) + ...
          fifth(:,:,4).*(3*nytarg.^2.*nx.^2.*ny+6.*nxtarg.*nytarg.*nx.*ny.^2+nxtarg.^2.*ny.^3)+ ...
          fifth(:,:,5).*(nytarg.*ny.^2.*(3*nytarg.*nx+2*nxtarg.*ny))+ ...
          fifth(:,:,6).*nytarg.^2.*ny.^3) + ... % G_{nx nx ny ny ny}
         -nu.*(fifth(:,:,1).*tauxtarg.^2.*nx.^3 + ...
          fifth(:,:,2).*(2*tauxtarg.*tauytarg.*nx.^3+3*tauxtarg.^2.*nx.^2.*ny) + ...
          fifth(:,:,3).*(tauytarg.^2.*nx.^3 + 6*tauxtarg.*tauytarg.*nx.^2.*ny+3.*tauxtarg.^2.*nx.*ny.^2) + ...
          fifth(:,:,4).*(3*tauytarg.^2.*nx.^2.*ny+6.*tauxtarg.*tauytarg.*nx.*ny.^2+tauxtarg.^2.*ny.^3)+ ...
          fifth(:,:,5).*(tauytarg.*ny.^2.*(3*tauytarg.*nx+2*tauxtarg.*ny))+ ...
          fifth(:,:,6).*tauytarg.^2.*ny.^3) + ... % nu*G_{taux taux ny ny ny}
        -a1.*(fifth(:,:,1).*nxtarg.^2.*nx.*taux.^2 + ...
          fifth(:,:,2).*(2.*nx.*nxtarg.^2.*taux.*tauy + 2.*nx.*nxtarg.*nytarg.*taux.^2 + nxtarg.^2.*ny.*taux.^2) + ...
          fifth(:,:,3).*(nx.*nxtarg.^2.*tauy.^2 + 4*nx.*nxtarg.*nytarg.*taux.*tauy + 2*nxtarg.^2.*ny.*taux.*tauy + nx.*nytarg.^2.*taux.^2 + 2*nxtarg.*ny.*nytarg.*taux.^2) + ...
          fifth(:,:,4).*(2*nx.*nxtarg.*nytarg.*tauy.^2 + nxtarg.^2.*ny.*tauy.^2 + 2*nx.*nytarg.^2.*taux.*tauy + 4*nxtarg.*ny.*nytarg.*taux.*tauy + ny.*nytarg.^2.*taux.^2) + ...
          fifth(:,:,5).*(nytarg.*tauy.*(2*ny.*nytarg.*taux + 2*nxtarg.*ny.*tauy + nx.*nytarg.*tauy)) + ...
          fifth(:,:,6).*ny.*nytarg.^2.*tauy.^2) + ...  % G_{nx nx ny tauy tauy}
        -nu*a1.*(fifth(:,:,1).*tauxtarg.^2.*nx.*taux.^2 + ...
          fifth(:,:,2).*(ny.*taux.^2.*tauxtarg.^2 + 2.*nx.*taux.*tauxtarg.^2.*tauy +2.*nx.*taux.^2.*tauxtarg.*tauytarg) + ...
          fifth(:,:,3).*(nx.*tauxtarg.^2.*tauy.^2 + 4*nx.*tauxtarg.*tauytarg.*taux.*tauy + 2*tauxtarg.^2.*ny.*taux.*tauy + nx.*tauytarg.^2.*taux.^2 + 2*tauxtarg.*ny.*tauytarg.*taux.^2) + ...
          fifth(:,:,4).*(2*nx.*tauxtarg.*tauytarg.*tauy.^2 + tauxtarg.^2.*ny.*tauy.^2 + 2*nx.*tauytarg.^2.*taux.*tauy + 4*tauxtarg.*ny.*tauytarg.*taux.*tauy + ny.*tauytarg.^2.*taux.^2) + ...
          fifth(:,:,5).*(tauytarg.*tauy.*(2*ny.*tauytarg.*taux + 2*tauxtarg.*ny.*tauy + nx.*tauytarg.*tauy)) + ...
          fifth(:,:,6).*ny.*tauytarg.^2.*tauy.^2) + ... % nu*G_{taux taux ny tauy tauy}
         a2.*kappa.*(forth(:, :, 1).*(nxtarg.*nxtarg.*nx.*nx) + ...
          forth(:, :, 2).*(2*nx.^2.*nxtarg.*nytarg + 2*nx.*nxtarg.^2.*ny) + ...
          forth(:, :, 3).*(nxtarg.^2.*ny.^2 + 4*nx.*nxtarg.*ny.*nytarg + nx.^2.*nytarg.^2) +...
          forth(:, :, 4).*(2*ny.*nytarg.*(nxtarg.*ny + nx.*nytarg))+...
          forth(:, :, 5).*(nytarg.*nytarg.*ny.*ny)) + ... % kappa*G_{nx nx ny ny}
         nu*a2.*kappa.*(forth(:, :, 1).*(tauxtarg.*tauxtarg.*nx.*nx) + ...
          forth(:, :, 2).*(2*nx.^2.*tauxtarg.*tauytarg + 2*nx.*tauxtarg.^2.*ny) + ...
          forth(:, :, 3).*(tauxtarg.^2.*ny.^2 + 4*nx.*tauxtarg.*ny.*tauytarg + nx.^2.*tauytarg.^2) +...
          forth(:, :, 4).*(2*ny.*tauytarg.*(tauxtarg.*ny + nx.*tauytarg))+...
          forth(:, :, 5).*(tauytarg.*tauytarg.*ny.*ny)) + ... % kappa*nu*G_{nx nx ny ny}
        -a3.*kp.*(third(:, :, 1).*(nxtarg.*nxtarg.*taux) + third(:, :, 2).*(nxtarg.*nxtarg.*tauy+ 2*nxtarg.*nytarg.*taux) +...
            third(:, :, 3).*(2*nxtarg.*nytarg.*tauy +nytarg.*nytarg.*taux) +...
            third(:, :, 4).*(nytarg.*nytarg.*tauy)) + ... % kp*G_{nx nx tauy}
         -nu*a3.*kp.*(third(:, :, 1).*(tauxtarg.*tauxtarg.*taux) + third(:, :, 2).*(tauxtarg.*tauxtarg.*tauy + 2*tauxtarg.*tauytarg.*taux) +...
            third(:, :, 3).*(2*tauxtarg.*tauytarg.*tauy + tauytarg.*tauytarg.*taux) +...
            third(:, :, 4).*(tauytarg.*tauytarg.*tauy)) ;  % kp*nu*G_{taux taux tauy}

    submat(r2 < 1e-8) = (nu - 1)*(12*kappa(r2 < 1e-8).^3*(nu^2 - nu + 4) + kpp(r2 < 1e-8)*(-5*nu^2 + 4*nu + 33))/(48*pi*(nu - 3)) ; % diagonal replacement

% supported plate kernels for plotting
case {'supported_plate_eval_old'}
    srcnorm = srcinfo.n;
    srctang = srcinfo.d;
    srcd2 = srcinfo.d2;
    coefs = varargin{1};
...
case {'s_general', 'single_general'} % general biharmonic single layer
   ...
   val1 = chnk.flex2d.helmdiffgreen(zk1, src, targ); 
   val2 = chnk.flex2d.helmdiffgreen(zk2, src, targ); 

   submat = 1/(zk1^2-zk2^2)*(val1 - val2);

case {'clamped_plate_bcs_general'}
    nxtarg = targinfo.n(1,:).'; 
    nytarg = targinfo.n(2,:).';  
    submat = zeros(2*nt,ns);
    ...
    [val1,grad1] = chnk.flex2d.helmdiffgreen(zk1, src, targ); 
    [val2,grad2] = chnk.flex2d.helmdiffgreen(zk2, src, targ); 
...
case {'d_general', 'double_general'} % normal derivative of flexural wave single layer
   ...
   [~, grad1] = chnk.flex2d.helmdiffgreen(zk1, src, targ);     
   [~, grad2] = chnk.flex2d.helmdiffgreen(zk2, src, targ);   
...
% discretized using log quadrature
    ...
    [~, grad1, hess1, third1] = gfunc1(src,targ);     
    [~, grad2, hess2, third2] = gfunc2(src,targ);     

    grad = 1/(zk1^2-zk2^2)*(grad1 - grad2);
    hess = 1/(zk1^2-zk2^2)*(hess1 - hess2);
    third = 1/(zk1^2-zk2^2)*(third1 - third2);

    K11 = -(third(:,:,1).*nx.^3 + 3*third(:,:,2).*nx.^2.*ny + 3*third(:,:,3).*nx.*ny.^2 + third(:,:,4).*ny.^3) + ...
         -a1*(third(:,:,1).*nx.*taux.^2 + third(:,:,2).*(ny.*taux.^2 + 2*nx.*taux.*tauy) + third(:,:,3).*(nx.*tauy.^2 + 2*ny.*taux.*tauy) + third(:,:,4).*ny.*tauy.^2) + ...
         a2*kappa.*(hess(:,:,1).*nx.^2 + 2*hess(:,:,2).*nx.*ny + hess(:,:,3).*ny.^2) + ...
         -a3*kp.*(grad(:,:,1).*taux + grad(:,:,2).*tauy);
    
    K12 = -(grad(:, :, 1).*nx + grad(:, :, 2).*ny);
           
    K22 = -(third(:,:,1).*(nx.*nxtarg.^2) + third(:,:,2).*(2*nx.*nxtarg.*nytarg + ny.*nxtarg.^2) + ...
         + third(:,:,3).*(nx.*nytarg.^2 + 2*ny.*nxtarg.*nytarg) + third(:,:,4).*(ny.*nytarg.^2))+...
         -nu.*(third(:,:,1).*(nx.*tauxtarg.^2) + third(:,:,2).*(2*nx.*tauxtarg.*tauytarg + ny.*tauxtarg.^2) + ...
         + third(:,:,3).*(nx.*tauytarg.^2 + 2*ny.*tauxtarg.*tauytarg) + third(:,:,4).*(ny.*tauytarg.^2)); 
    
    [~, ~, ~, third1, fourth1, fifth1] = gfunc1f(src,targ);  
    [~, ~, ~, third2, fourth2, fifth2] = gfunc2f(src,targ);   

    third = 1/(zk1^2-zk2^2)*(third1 - third2);    
    forth = 1/(zk1^2-zk2^2)*(fourth1 - fourth2);    
    fifth = 1/(zk1^2-zk2^2)*(fifth1 - fifth2);    
    
    K21 = -(fifth(:,:,1).*nxtarg.^2.*nx.^3 + ... % K21 with the biharmonic Green's function subtracted off (biharmonic part handled in a separate function)
          fifth(:,:,2).*(2*nxtarg.*nytarg.*nx.^3+3*nxtarg.^2.*nx.^2.*ny) + ...
          fifth(:,:,3).*(nytarg.^2.*nx.^3 + 6*nxtarg.*nytarg.*nx.^2.*ny+3.*nxtarg.^2.*nx.*ny.^2) + ...
          fifth(:,:,4).*(3*nytarg.^2.*nx.^2.*ny+6.*nxtarg.*nytarg.*nx.*ny.^2+nxtarg.^2.*ny.^3)+ ...
          fifth(:,:,5).*(nytarg.*ny.^2.*(3*nytarg.*nx+2*nxtarg.*ny))+ ...
...
% supported plate kernels for plotting
    ...
    K1 = -(third(:,:,1).*nx.^3 + 3*third(:,:,2).*nx.^2.*ny + 3*third(:,:,3).*nx.*ny.^2 + third(:,:,4).*ny.^3) + ...
         -a1*(third(:,:,1).*nx.*taux.^2 + third(:,:,2).*(ny.*taux.^2 + 2*nx.*taux.*tauy) + third(:,:,3).*(nx.*tauy.^2 + 2*ny.*taux.*tauy) + third(:,:,4).*ny.*tauy.^2) + ...
         a2*kappa.*(hess(:,:,1).*nx.^2 + 2*hess(:,:,2).*nx.*ny + hess(:,:,3).*ny.^2) + ...
         -a3*kp.*(grad(:,:,1).*taux + grad(:,:,2).*tauy);

    K2 = -(grad(:,:,1).*nx + grad(:,:,2).*ny);

...
%         Returns an (nt x ns) matrix (or (opdims(1)*nt) x (opdims(2)*ns)
%         for non-scalar kernels).
%
%      K.getquads(S, eps, dpars, targinfo, opts)
%         Function handle.  Returns the near-quadrature correction struct
%         Q used by the layer-potential evaluator.
%
%      K.fmm(S, sigma, targinfo, eps, dpars)
%         Function handle.  Evaluates the layer potential via the
%         FMM-accelerated evaluator.
%
%      K.sing   - singularity type: 'smooth', 'log', 'pv', 'hs'
%      K.opdims - [m n], operator dimensions (scalar kernels: [1 1])
%      K.params - struct of kernel parameters (e.g. coefs)
%
% See also LAP3D.KERN, LAP3D.DIRICHLET.EVAL, LAP3D.DIRICHLET.GET_QUADRATURE_CORRECTION

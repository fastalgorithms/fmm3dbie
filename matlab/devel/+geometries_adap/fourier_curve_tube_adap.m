function S = fourier_curve_tube_adap(coefs, nuv, norder, iptype,...
    tol, lptype, eta)
% radii = [a,b,r]
% !   a : major radius / offset from z-axis to center of torus tube
% !   b : minor radius of the underlying torus
% !   c : frequency in the poloidal direction (controls cos(c*s), sin(c*s))
% !   d : frequency in the toroidal direction (controls cos(d*s), sin(d*s))
% !   r : radius of the tubular cross-section around the center curve
% torus_curve_tube Create discretized torus surfer deformed toroidal Fourier mode.
%
...
%
  ...
  if nargin < 2 || isempty(nuv)
    nuv = [5;5];
  end

  if nargin < 3 || isempty(norder)
    norder = 4;
  end

  if nargin < 4 || isempty(iptype)
    iptype = 1;
  end

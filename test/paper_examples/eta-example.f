 ...
 1120 format(2x,i2,2(2x,e11.5))
     ...
     1    srccoefs,srcvals,wts,pdis)
        ...
        open(unit=34,file='eta-res/stell20_eta_asp.txt',
     1       access='append')
        write(34,1120) norder,pavg,pmax
        close(34)

        if(norder.eq.7) then
          open(unit=35,file='eta-res/stell20_asp_hist.txt')
          do i=1,npatches
            write(35,*) i,pdis(i)
          enddo
        endif
...
C$          t2 = omp_get_wtime()          
          ...
          open(unit=33,file='eta-res/stell20_eta_mac_iprec2.txt',
     1       access='append')
          write(33,1110) norder,rfac,npatches,npts,

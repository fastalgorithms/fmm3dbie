...
subroutine lap_bel_res(src, ndt,targ, ndd,dpars,ndz,zk,ndi,ipars,val)
  ...
  !
  ! returns the Laplace-Beltrami operator applied to the logarithmic
  ! kernel, i.e.
  !
  !   K(x,y) = 4*(n(x).(x-y)/|x-y|^2)*(n(x).(x-y)/|x-y|^2 - H(x))
  !
...
subroutine lap_bel_log(src, ndt,targ, ndd,dpars,ndz,zk,ndi,ipars,val)
  ...
  data over4pi/0.07957747154594767d0/
  !
  ! returns the logarithmic kernel log|x-y|^2/(4pi), the parametrix for
...
subroutine helm_bel_res(src, ndt,targ, ndd,dpars,ndz,zk,ndi,ipars,zval)
  ...
  data over4pi/0.07957747154594767d0/
  !
  ! returns the Helmholtz-Beltrami operator applied to the Hankel
  ! kernel, i.e.
  !
  !   K(x,y) = (-zk^2*H_2(zk*r)*(n(x).(x-y)/r)^2
  !              + 2*H(x)*zk*H_1(zk*r)*(n(x).(x-y)/r))/(4i)
  !
...
subroutine helm_bel_hank(src, ndt,targ, ndd,dpars,ndz,zk,ndi,ipars,zval)
  ...
  data ima/(0.0d0,1.0d0)/
  !
  ! returns the Hankel kernel H_0^{(1)}(zk*|x-y|)/(4i), the parametrix
  ! for the Helmholtz-Beltrami operator
  !

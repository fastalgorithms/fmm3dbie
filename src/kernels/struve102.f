...
c
     ...
     1      sk0xxx,sk0xxy,sk0xyy,sk0yyy)
        ...
        complex *16 cr0xxx,cr0xxy,cr0xyy,cr0yyy
        complex *16 cr1xxx,cr1xxy,cr1xyy,cr1yyy
        complex *16 h0xx,h0xy,h0yy,h0xxx,h0xxy
        complex *16 h0xyy,h0yyy
        complex *16 sk0xx,sk0xy,sk0yy
        complex *16 sk0xxx,sk0xxy,sk0xyy,sk0yyy
        real *8 dx,dx2,dy,dy2,dr,dr2,dr3,dr4,dr5,pi
        logical ilow
        data ima /(0,1)/
     ...
     1      h0xy,h0yy,h0xxx,h0xxy,h0xyy,h0yyy)
        ...
        cr1x = rhoj*dx/dr*cr0 - dx/dr2*cr1
        cr1y = rhoj*dy/dr*cr0 - dy/dr2*cr1
        cr0xx = - rhoj*dy2/dr3*cr1 - rhoj*dx/dr*cr1x
        cr0xy = rhoj*dx*dy/dr3*cr1 - rhoj*dx/dr*cr1y
        cr0yy = - rhoj*dx2/dr3*cr1 - rhoj*dy/dr*cr1y

        sk0xx = -ima*cr0xx+ima*h0xx
        sk0xy = -ima*cr0xy+ima*h0xy
        sk0yy = -ima*cr0yy+ima*h0yy


        cr1xx = rhoj*dy2/dr3*cr0 + rhoj*dx/dr*cr0x 
     1           + (dx2-dy2)/dr4*cr1 - dx/dr2*cr1x
        cr1xy = - rhoj*dx*dy/dr3*cr0 + 2*dx*dy/dr4*cr1
     1           + rhoj*dx/dr*cr0y - dx/dr2*cr1y
        cr1yy = rhoj*dx2/dr3*cr0 + rhoj*dy/dr*cr0y 
     1          + (dy2-dx2)/dr4*cr1 - dy/dr2*cr1y


        cr0xxx = 3*rhoj*dy2*dx/dr5*cr1 - 2*rhoj*dy2/dr3*cr1x
     1           - rhoj*dx/dr*cr1xx
        cr0yyy = 3*rhoj*dx2*dy/dr5*cr1 - 2*rhoj*dx2/dr3*cr1y
     1           - rhoj*dy/dr*cr1yy
        cr0xxy = rhoj*dy*(dy2 - 2*dx2)/dr5*cr1 + rhoj*dx*dy/dr3*cr1x
     1           - rhoj*dy2/dr3*cr1y - rhoj*dx/dr*cr1xy
        cr0xyy = rhoj*dx*(dx2 - 2*dy2)/dr5*cr1 + rhoj*dy*dx/dr3*cr1y
     1           - rhoj*dx2/dr3*cr1x - rhoj*dy/dr*cr1xy
       
       
        sk0xxx = -ima*cr0xxx+ima*h0xxx
        sk0xxy = -ima*cr0xxy+ima*h0xxy
        sk0xyy = -ima*cr0xyy+ima*h0xyy
        sk0yyy = -ima*cr0yyy+ima*h0yyy

        if (ilow) then
            sk0xx = dconjg(sk0xx)
            sk0xy = dconjg(sk0xy)
            sk0yy = dconjg(sk0yy)
            sk0xxx = dconjg(sk0xxx)
            sk0xxy = dconjg(sk0xxy)
            sk0xyy = dconjg(sk0xyy)
            sk0yyy = dconjg(sk0yyy)
            rhoj = dconjg(rhoj)
        endif

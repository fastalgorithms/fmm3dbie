...
c                    point on
c
c  * dtriaints_wnodes: compute integral using prescribed nodes and 
c                      weights
c                       
c
...
c
* dtriaints_wnodes_vec: compute integral using prescribed nodes and 
c                       weights. 
c                       Vectorized version of dtriaints_wnodes
...
c
     ...
     3   ndi, ipars, nqpts, qnodes, wts, dintvals)
c
...
c               
c        In this routine the output is expected to be a 
c        real scalar
c    - ndd: integer *8
...
c 
c  Output arguments:
c    - dintvals: real *8 (nppols, ntarg)
...
c 
      ...
      real *8, intent(out) :: dintvals(nppols,ntarg)
...
c
     ...
     1         ndz,zpars,ndi,ipars,fval)
      ...
      call dgemm_guru(transa, transb, nppols, ntarg0, nqpts,
     1   alpha_c, sigvals, nppols, xkernvals, ntarg0, beta_c, 
     2   dintvals(1,itargptr(ipatch)), nppols)
...
c
      ...
      subroutine dtriaints_dist(eps, intype, npatches, norder, npols,
     ...
     4     ndi, ipars, nqorder, rfac, dintvals, ier)
c
c  Compute the integrals in (1) defined at the top of the file
...
c
     ...
     1             ndd, dpars, ndz, zpars, ndi, ipars, fkervals(ii))
          ...
          call dgemv_guru('n', nppols, npts, alpha_c, sigmatmp,
     1      nppols, fkervals, int8_1, beta_c, dintvals(1,itarg), int8_1)
        enddo
      enddo
...
c
 ...
 1111     continue
          ier = 0
          call dtriaadap(eps, nqorder, nqpols, nlmax, ntmaxuse,
...
c
      ksigpols = nppols
      allocate(dtmp(nppols))
      allocate(istack(2*ntmax))
      allocate(dvals(ksigpols,ntmax))
...
c
     ...
     1     ndi, ipars, fval)
         ...
         do j=1,ksigpols
            dvals(j,1) = dvals(j,1)+xkernvals(i)*sigvalsdens(j,i)
         enddo
      enddo
     ...
     3      ndz, zpars, ndi, ipars, dvals, istack, nproclist0,
     4      xkernvals, dintall, ier)
...
c
      ...
      subroutine dtriaadap_main(eps, kpols, nlmax, ntmax, 
     ...
     3    zpars, ndi, ipars, dvals, istack, nproclist0, xkernvals,
     4    dintall, ier)
      ...
      real *8 xt(ndtarg)
      real *8 srcvals(nds,*)
      real *8 dintall(nppols),fval,dtmp(nppols)
      real *8 dvals(nppols,ntmax)
...
c               print *, "Too many triangles in ddtriaadap"
...
c          triangle
          do isig=1,ksigpols
            dintall(isig) = dintall(isig)-dvals(isig,itri)
            dtmp(isig) = 0
          enddo
...
c
          do itric=itric1,itric1+3
            ...
            do k=1,kpols
              ii = istart+k
              do isig=1,ksigpols
                dvals(isig,itric) = dvals(isig,itric)+xkernvals(ii)*
     1                                  sigvalsdens(isig,ii)
              enddo
            enddo
              
            do isig=1,ksigpols
              dintall(isig) = dintall(isig) + dvals(isig,itric)
              dtmp(isig) = dtmp(isig)+dvals(isig,itric)
            enddo
          enddo
...
c
          errmax = 0
          do isig=1,ksigpols
            if(abs(dtmp(isig)-dvals(isig,itri)).gt.errmax) 
     1          errmax = abs(dtmp(isig)-dvals(isig,itri))
          enddo
...
c
     ...
     1              ndd, dpars, ndz, zpars, ndi, ipars, xkernvals(jj))
                ...
                do j=1,nppols
                  dd = xkernvals(jj)*sigvalsdens(j,jj)
                  dvals(j,iqtri) = dvals(j,iqtri)+dd
                  dintvals(j,itarg)=dintvals(j,itarg)+dd
                enddo
              enddo
            endif
          enddo
...
c 
      ...
      real *8, intent(out) :: dintvals(nd,nppols,ntarg)
...
c
        ...
        allocate(dinttmp(nppols, nd, ntarg0))
        do j=1,nqpts
          do itarg=itargptr(ipatch),itargptr(ipatch)+ntarg0-1
            ii = itarg - itargptr(ipatch)+1
     ...
     1         ndd, dpars, ndz, zpars, ndi, ipars, fval)
        ...
        call dgemm_guru(transa, transb, nppols, nd*ntarg0, nqpts,
     ...
     2     dinttmp, nppols)
        ...
        do itarg=itargptr(ipatch), itargptr(ipatch)+ntarg0-1
           do idim=1,nd
             do j=1,nppols
                ii = itarg -itargptr(ipatch)+1
                dintvals(idim,j,itarg) = dinttmp(j,idim,ii)
             enddo
          enddo 
        enddo
...
c  is the radius of the smallest sphere.
c
c  Vectorized version of dtriaints_dist 
c
c  Input arguments:
c    - eps: real *8
c        requested tolerance. Currently unsued, nqorder and rfac,
...
c        scaling factor for determining when a triangle 
c        needs refinement
c    - dintvals: real *8 (nd, nppols, ntarg)
c        the computed integrals
...
c
      ...
      real *8 dintvals(nd,nppols,ntarg)
...
c
     ...
     1      fkervals, nd, sigmatmp, nppols, beta_c, dintvals(1,1,itarg),
     2      nd)
...
c  on the triangle to a tolerance \eps.
c
c  Vectorized version of dtriaints_adap
c
c
c  Input arguments:
c    - eps: real *8
...
c
 ...
 1111     continue
          ...
          call dtriaadap_vec(eps, nqorder, nqpols, nlmax, ntmaxuse,
     1       ntri, ichild_start, tvs, da, uvsq, wts, 
...
c  adaptive integration routine.
c
c  Vectorized version of dtriaadap.
c
c
c  Input arguments:
c    - eps: real *8
...
c
      ksigpols = nppols
      allocate(istack(2*ntmax))
      allocate(dvals(nd,ksigpols,ntmax))
...
c
     ...
     1           xkernvals(idim,i)*sigvalsdens(j,i)
      ...
      call dtriaadap_main_vec(eps, kpols, nlmax, ntmax, ntri, 
     1       ichild_start, tvs, da, uvsq, wts, norder, npols,
     2       isd, ndsc, srccoefs, npmax, nds, srcvals, qwts, sigvals, 
     3       nporder, nppols, sigvalsdens, ndtarg, xt, rat1, rat2, rsc1,
     4       rat1p, rat2p, rsc1p, fker, nd, ndd, dpars, ndz,
     5       zpars, ndi, ipars, dvals, istack, nproclist0,
     6       xkernvals, dintall, ier)
...
c
     ...
     5      ipars, dvals, istack, nproclist0, xkernvals, dintall, ier)
      ...
      real *8 xt(ndtarg)
      real *8 srcvals(nds,*)
      real *8 dintall(nd,nppols),fval(nd),dtmp(nd,nppols)
      real *8 dvals(nd,nppols,ntmax)
...
c
     ...
     2              xkernvals(1,jj))
                ...
                do j=1,nppols
                  do idim=1,nd
                    dd = xkernvals(idim,jj)*sigvalsdens(j,jj)
                    dvals(idim,j,iqtri) = dvals(idim,j,iqtri)+dd
                    dintvals(idim,j,itarg)=dintvals(idim,j,itarg)+dd
                  enddo
                enddo
              enddo
            endif

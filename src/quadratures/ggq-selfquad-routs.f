...
cc it under the terms of the GNU General Public License version 2 as published by 
cc the Free Software Foundation.
...
c
c                            Input Parameters:
c   irad - parameter for which radial quadratures to use,
...
c
        ...
        double precision ts(250),twhts(250)
c
...
c
        nlege = 4*irad/2+1
        call get_rad_lege(irad,nlege,xlege,wlege)
...
c      
        ...
        ier = 128
        call prinf('ggq-rad:one of the vertices too close to origin*',
     1     i,0)
        return
        endif
c
...
c
        ipv = 0
        call get_radfetch_param(ipv,irad,r,t,nt,ir,it)
        call radfetch(ipv,irad,ir,it,nt,ts,twhts)
...
c
        call raddiag1(nt,ts,twhts,nlege,xlege,wlege,amatr,nquad,
     1    xs,ys,whts,r,t)
c
        return
        end
...
c        available with irad=4, and so on
c
c    - verts: double precision(2,3)
c        a (2,3) array each column of which gives the coordinates
c        of one vertex of the user-supplied triangle
c
c  Output Parameters:
c
...
c
      nquad0 = nlege*n
      call pv_raddiag_disk0(irad,n,nlege,rcut,
...
c
      ...
      call get_rad_lege(irad,nlege,xslege,whtslege)
c
      r0 = 0
      r1 = rcut
      ii = 0
...
c
      ipv = 1
      call get_radfetch_param(ipv,irad,alpha,phi,nt,ir,it)
      call radfetch(ipv,irad,ir,it,nt,ts,twhts)
c
c
c       Call an auxillary routine to build the product quadrature.
c
      norder = 4*irad
      call pv_raddiag1(nt,ts,twhts,nquad,xs,ys,whts,alpha,phi,
     1   rcut0,norder)
c
      do i=1,nquad
        x = xs(i)
...
c
c
      subroutine get_rad_lege(irad,nlege,ts,wts)
c------------
c  This subroutine returns precomputed 
c  gauss-legendre nodes and weights needed for radfetch
c 
c  Input arguments:
c
c    - irad: integer
...
c  Output arguments:
c   
c    - ts: double precision(nlege)
c        legendre nodes of order nlege
c         
c    - wts: double precision(nlege)
c        legendre weights of order nlege
c         
c
      ...
      if(irad.eq. 1) then
        ts(  1) =-.7745966692414833D+00
        ts(  2) =0.0000000000000000D+00
        ts(  3) =0.7745966692414833D+00

        wts(  1) =0.5555555555555551D+00
        wts(  2) =0.8888888888888888D+00
        wts(  3) =0.5555555555555551D+00
      endif

      if(irad.eq. 2) then
        ts(  1) =-.9061798459386640D+00
        ts(  2) =-.5384693101056830D+00
        ts(  3) =0.0000000000000000D+00
        ts(  4) =0.5384693101056830D+00
        ts(  5) =0.9061798459386640D+00

        wts(  1) =0.2369268850561891D+00
        wts(  2) =0.4786286704993662D+00
        wts(  3) =0.5688888888888889D+00
        wts(  4) =0.4786286704993662D+00
        wts(  5) =0.2369268850561891D+00
      endif

      if(irad.eq. 3) then
        ts(  1) =-.9491079123427585D+00
        ts(  2) =-.7415311855993943D+00
        ts(  3) =-.4058451513773971D+00
        ts(  4) =0.0000000000000000D+00
        ts(  5) =0.4058451513773971D+00
        ts(  6) =0.7415311855993943D+00
        ts(  7) =0.9491079123427585D+00

        wts(  1) =0.1294849661688697D+00
        wts(  2) =0.2797053914892766D+00
        wts(  3) =0.3818300505051188D+00
        wts(  4) =0.4179591836734694D+00
        wts(  5) =0.3818300505051188D+00
        wts(  6) =0.2797053914892766D+00
        wts(  7) =0.1294849661688697D+00
      endif
...
c
c
      subroutine get_radfetch_param(ipv,irad,r,t,n,ir,it)
c---------------------
c
c  this subroutine determines the quadrature 
c  region number corresponding to parameters
c  r,t,and irad
c
c  Input arugments:
c
...
c         radial coordinate in triangle
c    - t: double precision
c         angle coordinate in triangle
c
c  Output arguments:
c
c    - n: integer
...
c
      ...
      if(irad.lt.1.or.irad.gt.3) then
...
c---------------------
c
c  this subroutine extracts the quadrature 
c  corresponding to region number ipv,irad,ir,it
c
c  Input arugments:
c
c    - ipv: integer
...
c  Output arguments:
c    
c    - ts: double precision(nt)
c        quadrature nodes
c    - wts: double precision(nt)

...
!    call pv_self_quadrature_new(ier, rad, vert1a, x0, y0, dxyzduv, &
!        ns, xs, ys, ws)



        subroutine pv_self_quadrature_new(ier,rad,verts0,x0,y0,dr,
     1       nquad,xs,ys,whts)
        implicit double precision (a-h,o-z)
        dimension verts(2,3),dr(3,2),a(2,2),ainv(2,2),verts0(2,3)
        dimension xs(*),ys(*),whts(1),b(3,2)
cccc        double precision, allocatable :: rad(:)
cccc        save rad
c
...
c
        ainv=0
        call pv_self_findmap(dr,a,ainv)
...
c
        call pv_raddiag(jer,rad,verts,nquad,xs,ys,whts)
c
...
c
        ...
        subroutine pv_self_findmap(a,b,binv)
        implicit double precision (a-h,o-z)

...
c    for error code in dquadadap_main, and then reallocating
...
c
c         
c  - dquadints_adap_vec: 
c       completely adaptive integration for the quadrangle.
c       vectorized verion of dquadints_adap
c   - dquadints_wnodes_vec: 
c       compute integrals with prescribed nodes and weights.
c       vectorized version of dquadints_wnodes
c  - dquadints_dist_vec: 
c       refine until targets/proxy separated
c       by quadrangle by rfac*r_{t} where r_{t} is the radius of 
c       enclosing sphere for quadrangle.
c       vectorized verison of dquadints_dist
c  - dquadints_comb_vec:
c      combination of adaptive integration and distance based refinement.
c      Use distance based refinement to compute the integrals on some 
c      coarse hierarchy of meshes and then use adaptive integration 
c      from that point on.
c      vectorized version of dquadints_comb
c               
...
c
      ...
      subroutine dquadints_wnodes(npatches,norder,ipoly,ttype,npols,
...
c
     ...
     1        ndz,zpars,ndi,ipars,fval)
          ...
          enddo
        enddo
        call dgemm_guru(transa,transb,nppols,ntarg0,nqpts,alpha,
     1    sigvals,nppols,xkernvals,nqpts,beta,
...
c
c
      subroutine dquadints_dist(eps,intype,
...
c
c
      subroutine dquadints_adap(eps,intype,
     1     npatches,norder,ipoly,ttype,npols,srccoefs,ndtarg,
...
c
 ...
 1111     continue
          ier = 0
          call dquadadap(eps,nqorder,nqpols,nlmax,nqmaxuse,nquad,
     ...
     7          cintvals(1,itarg),ier)
           if(ier.eq.4) then
             ...
             nn3 = 12*npmax
             nn4 = nquad*6
             call dcopy(nn1,sigvals,int(1,8),sigvals2,int(1,8))
             call dcopy(nn2,sigvalsdens,int(1,8),sigvalsdens2,int(1,8))
             call dcopy(nn3,srcvals,int(1,8),srcvals2,int(1,8))
             call dcopy(npmax,qwts,int(1,8),qwts2,int(1,8))
             do ii=1,nquad
               ichild_start2(ii) = ichild_start(ii)
             enddo
             call dcopy(nn4,tvs,int(1,8),tvs2,int(1,8))
             call dcopy(nquad,da,int(1,8),da2,int(1,8))
             ...
             call dcopy(nn1,sigvals2,int(1,8),sigvals,int(1,8))
             call dcopy(nn2,sigvalsdens2,int(1,8),sigvalsdens,int(1,8))
             call dcopy(nn3,srcvals2,int(1,8),srcvals,int(1,8))
             call dcopy(npmax,qwts2,int(1,8),qwts,int(1,8))
             do ii=1,nquad
               ichild_start(ii) = ichild_start2(ii)
             enddo
             call dcopy(nn4,tvs2,int(1,8),tvs,int(1,8))
             call dcopy(nquad,da2,int(1,8),da,int(1,8))
...
c
c
      subroutine dquadadap(eps,m,kpols,nlmax,nqmax,nquad,
     1             ichild_start,tvs,da,uvsq,wts,
...
c
     ...
     1      ipars,fval)
      ...
      call dquadadap_main(eps,kpols,nlmax,nqmax,nquad,ichild_start,
...
c               print *, "Too many quadrangles in cdquadadap"
...
c
c
      subroutine dquadints_comb(eps,intype,
     1     npatches,norder,ipoly,ttype,npols,srccoefs,ndtarg,
     2     ntarg,xyztarg,ifp,xyzproxy,
     3     itargptr,ntargptr,nporder,nppols,nquadmax,
...
c
          ...
          ier = 0
          call dquadadap_main(eps,nqpols,nlmax,nquadmax,nquad,

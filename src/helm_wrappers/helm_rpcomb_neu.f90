...
!
!
!    - lpcomp_rpcomb_neu_eval_addsub: Compute the solution
...
!  Temporary variables      
  ...
  !$OMP END PARALLEL DO
        ...
        call zgetnearquad_ggq_guru(npatches, norders, ixyzs, &
          ...
          ipatch_id, uvs_targ, eps, ipv, fker, ndd, dpars, ndz, &
          zpars_tmp, ndi, ipars, nnz, row_ptr, col_ind, iquad, &
...
!      
!
      subroutine lpcomp_helm_rpcomb_neu_eval_addsub(npatches, norders, &
        ixyzs, iptype, npts, srccoefs, srcvals, ndtarg, ntarg, targs, &
...
!  extract source and target info
!
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(rtmp, ztmp)
      do i=1,ns
        rtmp = whtsover(i)*over4pi
        ztmp = rtmp*ima*zpars(2)

        sources(1,i) = srcover(1,i)
        sources(2,i) = srcover(2,i)
        sources(3,i) = srcover(3,i)
        charges(i) = sigmaover(1,i)*rtmp
        dipvec(1,i) = sigmaover(2,i)*srcover(10,i)*ztmp
        dipvec(2,i) = sigmaover(2,i)*srcover(11,i)*ztmp
        dipvec(3,i) = sigmaover(2,i)*srcover(12,i)*ztmp
      enddo
!$OMP END PARALLEL DO      
...
!$OMP PRIVATE(jstart, npols, l)
      do i=1,ntarg
        do j=row_ptr(i),row_ptr(i+1)-1
          ...
          do l=1,npols
            pot(i) = pot(i) + &
              wnear(1,jquadstart+l-1)*sigma(1,jstart+l-1)
            pot(i) = pot(i) + &
              wnear(2,jquadstart+l-1)*sigma(2,jstart+l-1)*ima*zpars(2)
          enddo
        enddo
      enddo
!$OMP END PARALLEL DO
...
!
!                        
      subroutine lpcomp_helm_rpcomb_eval(npatches, norders, ixyzs, &
        iptype, npts, srccoefs, srcvals, ndtarg, ntarg, targs, &
...
! evaluate the layer potential
      ...
      call lpcomp_helm_rpcomb_neu_eval_addsub(npatches, norders, &
        ixyzs, iptype, npts, srccoefs, srcvals, ndtarg, ntarg, targs, &
        ipatch_id, uvs_targ, eps, ndd, dpars, ndz, zpars, ndi, ipars, &
        nnz, row_ptr, col_ind, iquad, nquad, nker, wnear, novers, &
      ...
      return
      end subroutine lpcomp_helm_rpcomb_eval

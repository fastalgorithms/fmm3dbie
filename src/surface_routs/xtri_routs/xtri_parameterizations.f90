...
subroutine surf_vals_to_coefs(nd,npatches,norder,npols,npts,umat, &
  vals,coefs)

!
!
!  this subroutine converts a function on a discretized surface
!  to its corresponding Koornwinder expansion coefficients
!  on each of the patches
!
!  input:
!    nd - integer
!       number of functions on the surface
!    npatches - integer
!       number of patches describing the surface
!    
!    norder - integer
...
!    
!    umat - real *8 (npols,npols)
!       matrix mapping values on a single patch to its corresponding
!       koornwinder expansion coefficients
!
!    vals - real *8 (nd,npts)
!       function values on the surface
!
!  output:
!    coeffs - real *8 (nd,npts)
!       koornwinder expansion coefficients of the functions on the
!       surface
  
  
  implicit none
  integer nd,npatches,norder,npols,npts
  real *8 umat(npols,npols),vals(nd,npols),coefs(nd,npols)
  integer ipatch,ipt,lpt,i,j,l
  ...
  do ipatch=1,npatches
    do i=1,npols
      ...
      do j=1,nd
        ...
        do l=1,npols
          lpt = (ipatch-1)*npols + l
          coefs(j,ipt) = coefs(j,ipt) + umat(i,l)*vals(j,lpt)
        enddo
      enddo
    enddo
  enddo

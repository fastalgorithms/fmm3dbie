...
ccc   sort each part
C$OMP PARALLEL DO DEFAULT(SHARED)
C$OMP$PRIVATE(ithread,nelems,istart,iend,ielems)
      do ithread = 1,nthreads
        istart = split(ithread)
        iend = split(ithread+1)
        nelems = iend-istart
c        write(*,*) "ithread:", ithread, "nelems: ", nelems, istart, iend
        call sortr(nelems,data(istart),index(istart))
      ...
      do while (j<nthreads)
        ...
        do while (i<nthreads+1)
          if(i+j<nthreads+1) then
            ...
            if(swap==1) then
              call merge_arr_para_r(data,index(istart),index(istart2),
     1             nelems,nelems2,index2(istart),nthreads)
            else
              call merge_arr_para_r(data,index2(istart),index2(istart2),
...
c
ccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine merge_arr_para_r(data,ind1,ind2,n1,n2,indout,nthreads)
C===================================================================
C
...
C$OMP$PRIVATE(i,j,k,indx,first,val,tmpk)
      do i=0,nthreads-1
        do j=0,ns-1
          ...
          call lower_bound_r(data,ind2,n2,val,first)
          ...
          call lower_bound_r(data,ind1,n1,val,first)
          split_size(k+1) = indx+first-1
        enddo
      enddo
C$OMP END PARALLEL DO
...
c        write(*,*) "i:",i,"n1:",n1tmp,"n2:",n2tmp
        call merge_arr_r(data,ind1(istart1),ind2(istart2),n1tmp,n2tmp,

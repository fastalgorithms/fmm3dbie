...
template <class Real, sctl::Integer MaxVecLen=4> void erf_svml_vec_cpp(const Real* x, const int64_t* n, Real* erfx) {
  /*
  sctl::Long N = n[0];
  sctl::Long N_ = ((N+MaxVecLen-1)/MaxVecLen)*MaxVecLen;
  static constexpr sctl::Integer VecLen = MaxVecLen;
  using Vec = sctl::Vec<Real, VecLen>;
  sctl::Vector<Real> X(N_);
  sctl::Vector<Real> V(N_);
  for (long i=0; i < N; i++){
    X[i] = x[i];
  }
  for (sctl::Long t = 0; t < N_; t += VecLen) {
    Vec xin;
    Vec xout;
    xin = Vec::LoadAligned(&X[t]);
    sctl::erf_intrin(xout, xin);
    xout.StoreAligned(&V[t]);
  }
  for (long i=0; i < N; i++){
    erfx[i] = V[i];
  }
  */
  ...
  static constexpr sctl::Integer VecLen = MaxVecLen;
  using Vec = sctl::Vec<Real, VecLen>;
  sctl::Vector<Real> X(BLK);
  sctl::Vector<Real> V(BLK);
  for (long sblk=0; sblk<N; sblk+=BLK){
    long cnt  = N - sblk;
    if(cnt > BLK) cnt = BLK;
    for (long t=sblk,i=0; i<cnt; t++,i++){
      X[i] = x[t];
    }
    for (long i=0; i<cnt; i+=VecLen){
      Vec xin;
      Vec xout;
      xin = Vec::LoadAligned(&X[i]);
      sctl::erf_intrin(xout, xin);
      xout.StoreAligned(&V[i]);
    }
    for (long t=sblk,i=0; i<cnt; t++,i++){
      erfx[t] = V[i];
    }
...
template <class Real, sctl::Integer MaxVecLen=4> void erf0_vec_cpp(const Real* x, const int64_t* n, Real* erfx) {
  sctl::Long N = n[0];
  static constexpr sctl::Integer VecLen = MaxVecLen;
  using Vec = sctl::Vec<Real, VecLen>;
  sctl::Vector<Real> X(BLK);
...
template <class Real, sctl::Integer MaxVecLen=4> void erf1_vec_cpp(const Real* x, const int64_t* n, Real* erfx) {
  sctl::Long N = n[0];
  static constexpr sctl::Integer VecLen = MaxVecLen;
  using Vec = sctl::Vec<Real, VecLen>;
  sctl::Vector<Real> X(BLK);
...
template <class Real, sctl::Integer MaxVecLen=4> void erf2_vec_cpp(const Real* x, const int64_t* n, Real* erfx) {
  sctl::Long N = n[0];
  static constexpr sctl::Integer VecLen = MaxVecLen;
  using Vec = sctl::Vec<Real, VecLen>;
  sctl::Vector<Real> X0(BLK);
  sctl::Vector<Real> V0(BLK);
  sctl::Vector<Real> X1(BLK);
  sctl::Vector<Real> V1(BLK);
  for (long sblk=0; sblk<N; sblk+=BLK){
    long cnt  = N - sblk;
    if(cnt > BLK) cnt = BLK;
    long cnt0 = 0;
    ...
    for (long i=0; i<cnt0; i+=VecLen){
      Vec xin;
      Vec xout;
      xin = Vec::LoadAligned(&X0[i]);
      sctl::erf0_intrin(xout, xin);
      xout.StoreAligned(&V0[i]);
    }
    for (long i=0; i<cnt1; i+=VecLen){
      Vec xin;
      Vec xout;
      xin = Vec::LoadAligned(&X1[i]);
      sctl::erf1_intrin(xout, xin);
      xout.StoreAligned(&V1[i]);
    }
    cnt0=0;
    cnt1=0;
    for (long t=sblk,i=0; i<cnt; t++,i++){
...
// tfmm3d dipole, potential and gradient
template <class Real, sctl::Integer MaxVecLen=4> void t3ddirectdg_vec_cpp(const int64_t* nd, 
        const Real* sources, const Real* dipvec, const int64_t* ns, 
        const Real* ztarg, const int64_t* nt, Real* pot, Real* grad, 
        const Real* thresh,
        const Real* qwt, const Real* sigma, const Real* sigma_grad) {
  static constexpr sctl::Integer COORD_DIM = 3;
  ...
  sctl::Matrix<Real> Xt(COORD_DIM, Ntrg_);
  sctl::Matrix<Real> Sigma_grad(COORD_DIM, Ntrg_);
  sctl::Matrix<Real> Sigma(1, Ntrg_, sctl::Ptr2Itr<Real>((Real*)sigma, Ntrg), true);
  sctl::Matrix<Real> Vt(nd_, Ntrg_);
  sctl::Matrix<Real> Gt(nd_*COORD_DIM, Ntrg_);
  { // Set Xs, Vs, Xt, Vt
    auto transpose = [](sctl::Matrix<Real>& A, const sctl::Matrix<Real>& B) {
    ...
    sctl::Matrix<Real> Xt_(Ntrg, COORD_DIM, sctl::Ptr2Itr<Real>((Real*)ztarg, COORD_DIM*Ntrg), false);
    sctl::Matrix<Real> Sigma_grad_(Ntrg, COORD_DIM, sctl::Ptr2Itr<Real>((Real*)sigma_grad, COORD_DIM*Ntrg), false);
    transpose(Xt, Xt_);
    transpose(Sigma_grad, Sigma_grad_);
    Vt = 0;
    Gt = 0;
  }
  ...
  // load dipstr, dipole
  sctl::Matrix<Real> Gs_(Nsrc, nd_*COORD_DIM,       sctl::Ptr2Itr<Real>((Real*)dipvec , nd_*Nsrc),       false);
  sctl::Matrix<Real> Vs_(Nsrc, nd_,                 sctl::Ptr2Itr<Real>((Real*)qwt , nd_*Nsrc),       false);
  sctl::Vector<Vec> Gsrc(Nsrc*nd_*COORD_DIM);
  sctl::Vector<Vec> Vsrc(Nsrc*nd_);
  for (sctl::Long s = 0; s < Nsrc; s++) {
    for (long i = 0; i < nd_; i++) {
  ...
  #pragma omp parallel for schedule(static)
  for (sctl::Long t = 0; t < Ntrg_; t += VecLen) {
    Vec Xtrg[COORD_DIM], Tsig, Tsigg[COORD_DIM];
    for (sctl::Integer k = 0; k < COORD_DIM; k++) {
      Xtrg[k] = Vec::LoadAligned(&Xt[k][t]);
      Tsigg[k] = Vec::LoadAligned(&Sigma_grad[k][t]);
    }
    Tsig = Vec::LoadAligned(&Sigma[0][t]);
    // load potential and gradient
    Vec Vtrg[nd_], Gtrg[nd_][COORD_DIM];
    for (long i = 0; i < nd_; i++) {
      Vtrg[i] = Vec::LoadAligned(&Vt[i][t]);
    ...
    for (sctl::Long s = 0; s < Nsrc; s++) {
      ...
      Vec R = Rinv*R2;
      Vec H,HH,HHH;
      myerf_intrin(H,HH,HHH,R,Tsig);

      for (long i = 0; i < nd_; i++) {
        Vec Dprod = dX[0]*Gsrc[s*nd_*COORD_DIM+i*COORD_DIM+0] + dX[1]*Gsrc[s*nd_*COORD_DIM+i*COORD_DIM+1] + dX[2]*Gsrc[s*nd_*COORD_DIM+i*COORD_DIM+2];
        Vtrg[i] -= Dprod*H*Vsrc[s*nd_+i];
        Gtrg[i][0] += (HH*dX[0]*Dprod + HHH*Dprod*Tsigg[0] + H*Gsrc[s*nd_*COORD_DIM+i*COORD_DIM+0])*Vsrc[s*nd_+i];
        Gtrg[i][1] += (HH*dX[1]*Dprod + HHH*Dprod*Tsigg[1] + H*Gsrc[s*nd_*COORD_DIM+i*COORD_DIM+1])*Vsrc[s*nd_+i];
        Gtrg[i][2] += (HH*dX[2]*Dprod + HHH*Dprod*Tsigg[2] + H*Gsrc[s*nd_*COORD_DIM+i*COORD_DIM+2])*Vsrc[s*nd_+i];
      }
    }
    for (long i = 0; i < nd_; i++) {
      Vtrg[i].StoreAligned(&Vt[i][t]);
